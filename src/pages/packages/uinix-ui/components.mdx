...
Implementing UI components with <PackageLink name="uinix-ui" /> is easy and usually involves fairly simple and terse code components as shown in the examples below.  All components can support variants through appropriate string concatenation of the `variant` prop in relation to how variants are defined in `system.styles` (see [§ `Button`](#button) for an example).
