...
The examples assume a preset `config` and `theme` detailed in [§ Appendix](#appendix).  You can adjust the examples appropriately with your configuration.  All examples are interactive and you can edit the code.  The examples outlined in this document are based on the list of [theme-ui components].  Feel free to open an [issue] if you want to suggest a component recipe to be added to the list!
