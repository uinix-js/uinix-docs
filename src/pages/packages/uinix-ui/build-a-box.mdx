...
In this guide, we will learn how to build and configure a `Box` component with <PackageLink name="uinix-ui" />.
...
The `Box` component is [popularized][theme-ui-box] by libraries such as [rebass] and [theme-ui].  It serves as a primitive component to build other components in UI systems.  We will use <PackageLink name="uinix-ui" /> to implement `Box`, supporting the following [theme-ui]-inspired features:
...
To support [shorthand style props][theme-ui-shorthand-style-props], let us first configure them for the <PackageLink name="uinix-ui" /> `Element` component.
...
In addition to configuring `Box` with [§ Shorthand style props](#shorthand-style-props), <PackageLink name="uinix-ui" /> provides further style customizations for `Box` via `config.elementStyles`.
...
In this guide, we covered building a `Box` component with <PackageLink name="uinix-ui" />.  Not only were we able to quickly implement popular features from libraries such as [theme-ui], we also explored that <PackageLink name="uinix-ui" />'s `Element` component behaves essentially as a configurable `Box` component.
